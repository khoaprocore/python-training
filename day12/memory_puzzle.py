...
ALLCOLOR = (RED, BLUE, YELLOW, PURPLE, CYAN, GREEN, ORANGE)
ALLSHAPE = (LINES, OVAL, DIAMOND, SQUARE, DONUT)
assert len(ALLCOLOR)*len(ALLSHAPE)*2 >= BOARDHEIGHT * BOARDWIDTH, "Board is too big for the number of shapes/colors defined."
...
def generateRevealedBoxesData(value):
    ...
    for i in range(BOARDWIDTH):
        revealedBoxes.append([value] * BOARDHEIGHT)
...
def coverBoxesAnimation(board, boxesToCover):
    for coverage in (0, BOXSIZE + REVEALSPEED, REVEALSPEED):
...
def main():
    ...
    mousex = 0
    mousey = 0
    pygame.display.set_caption("Memory Puzzel")
